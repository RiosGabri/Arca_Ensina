...
> **Banco de dados:** SQLite em desenvolvimento — não precisa instalar PostgreSQL.
...
2. Cadastre-se com perfil Médico (admin/pesquisador são bloqueados no auto-cadastro)
3. Confirme redirecionamento para `/dashboard`
4. Faça logout e login novamente
5. No DevTools (Network): confirme `POST /api/v1/auth/login/` → 200, `GET /api/v1/auth/user/` → 200

---

## Adicionando um domínio novo

Cada domínio vira um **app Django próprio**. Não misture domínios em apps existentes.
...
1. Registre em `INSTALLED_APPS` (`project/settings.py`)
2. Defina models — use `settings.AUTH_USER_MODEL` para FK de usuário
3. Crie serializers herdando de `project.serializers.BaseSerializer`
4. Crie views com `audit.mixins.AuditableMixin` antes de `ModelViewSet`
5. Crie `urls.py` e inclua em `project/urls.py` sob `api/<str:version>/`
6. Gere e aplique migrations
...
- Não duplique permissões — importe de `accounts/permissions.py`
...
| Login dá erro | Rode `python manage.py migrate` |
